...
class ConversationGenerator:
    ...
    def add_speaker_and_signal(self, source: List):
        ...
        for i, sentence in enumerate(source):
            ...
            value = sentence["value"] #问题和回答

            conv.append_message(role, value)

        processed = conv.process() #messages:[['human',"xxx"],[gpt,xxxx]]
        conversation, to_predict_list = processed['conv'], processed['to_predict']  #conversation:字符串，to_predict_list:list
...
class FinetuneDialogDataset(Dataset):
        ...
        if need_collect_anno:
            ...
            for meta in self.config['META']:
                ...
                if meta_ext == ".json":
                    ...
                    # with open("/mnt1/wgk/LLaMA2-Accessory/accessory/backup/con.json", "w") as f:
                    #     json.dump(meta_l[:10], f, indent=4)
                elif meta_ext == ".jsonl":
                    meta_l = []
                    with open(meta_path) as f:
    ...
    def get_item_func(self, index):
        ...
        tokenized_conversation = self.tokenizer.encode(conversation, bos=True, eos=True) #tokenize
        labels = [IGNORE_INDEX for _ in tokenized_conversation] #初始化一个与 tokenized_conversation 长度相同的列表，且该列表中的所有元素都被设置为 IGNORE_INDEX

        check_pos = 0
        for value in to_predict_values:
            tokenized_value = self.tokenizer.encode_segment(value) #list(int)
            value_pos = find_sublist(tokenized_conversation[check_pos:], tokenized_value) + check_pos #相同子列表最后一位
            if value_pos == -1:
        ...
        input2 = torch.tensor(tokenized_conversation, dtype=torch.int64) #complete conversation human:xxx ##gpt:xxx
        labels = torch.tensor(labels, dtype=torch.int64) #要预测的部分变为正数
        ...
        #把input和label的负数全部变成0
        input2_mask = input2.ge(0)
        label_mask = labels.ge(0)
        input2[~input2_mask] = 0
        labels[~label_mask] = 0 #input里面要预测的部分的索引
        input2_mask = input2_mask.float() #bool转float [0.0,0.0,1.0,0.0]
